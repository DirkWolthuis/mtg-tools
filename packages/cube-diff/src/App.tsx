...
/** Fetches both cube snapshots (current + as of `date`) from CubeCobra and diffs them. */
async function loadDiff(
  cubeId: string,
  date: string,
): Promise<{ diff: CubeDiff; cubeName: string; cubeLink: string }> {
  ...
  return {
    diff: diffCubes(oldCube.cards.mainboard, newCube.cards.mainboard),
...
export function App() {
  ...
  async function runDiff(values: CubeDiffFormValues) {
    ...
    try {
      const result = await loadDiff(cubeId, values.date);
      setState({ status: 'success', ...result, cubeId, date: values.date });
    } catch (error) {
      setState({
        status: 'error',
        message:
  ...
  async function runShareView(share: ShareDiffState) {
    setState({ status: 'loading' });
    try {
      const result = await loadDiff(share.cubeId, share.date);
      setState({
        status: 'success',
        ...result,
        cubeId: share.cubeId,
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-6 p-6">
      <header>
        <h1 className="text-2xl font-bold">CubeCobra visual changelog</h1>
      ...
      {shareUrl && !shareState && <ShareLink url={shareUrl} />}
